...
```{r data to matrix}
BC.reads <- indexed.reads[,6:8] # only eDNA index values
BC.mat<- t(BC.reads) # columns = PCR rep number, rows = Hashes, values = nReads
```
...
```{r BC test}
vegdist(BC.mat, method="bray", na.rm = T)
```
...
```{r Beta Div test}
vegan::betadiver(dataset, method = 1)
```
