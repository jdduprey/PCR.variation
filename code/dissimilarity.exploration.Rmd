...
```
All of LK samples are showing up as NA, and it seems like a K+ and k+ also showed up somehow. 
...
bray.output <-   bray_by_sample(full.indexed.reads, unique(full.indexed.reads$bio), c(6:8))

par(mfrow = c(1, 3))
hist(bray.output[,1], main="PCRs 1 & 2", xlab="BCD", ylim=c(0,80)) 
hist(bray.output[,2], main="PCRs 2 & 3", xlab="BCD", ylim=c(0,80)) 
hist(bray.output[,3], main="PCRs 1 & 3", xlab="BCD", ylim=c(0,80))
```
...
Hm p value of 0.4238. So at very first glance, reads don't differ that much between PCR replicates. But I still need to learn more about the adonis function to see if this is an appropriate test. 
