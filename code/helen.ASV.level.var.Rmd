...
for(i in 1:length(nested.props$sample)){
  nested.props$data[[i]] <-  nested.props$data[[i]] %>% mutate(grouped_id = row_number()) %>% 
pivot_wider(names_from = bio.PCR, values_from= Proportion, values_fill = 0) %>% select(-grouped_id)
  print(i)
}
```
