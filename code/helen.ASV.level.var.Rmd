...
Now I want to find out which samples (rows of long.reads.C) have > or < 3 biological replicates. 

```{r Number of Biological replicates}
for(i in 1:length(nested.props$sample)){
  # how many unique bio.PCRs (A.1, E.3, etc) there are in each sample, divide by 3 to see how many don't have 3 bio replicates
  print(c(i,nrow(unique(nested.props$data[[i]][,1]))/3))
}

#1:6, 23,30,44:46, 59,63,67,69,71,74,76
```
