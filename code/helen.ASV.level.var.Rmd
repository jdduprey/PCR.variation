...
Part D : Site & Date Variation (in progress)
...
# Part C : Bottle ASV Variation (Proportions averaged across PCR replicates)

I just stole this from Joe's Joe.mean.ASV.level.Rmd, with a few minor additions. 1st code block gets mean proportion for each hash within each biological replicate.
...
Side note: Mean proportions within 1 bottle (ex, CP201706.A) shouldn't really add up to 1, because they comprise the means across PCRs (so not exact). But they should be close. Quick test:

```{r Proportion sum test}
propsums <- get.PCRn %>% group_by(biorep) %>% summarise(sum = sum(meanprop) )
hist(propsums$sum)
length(which(propsums$sum == 1))/length(propsums$sum) # 78% of proportions sum to 1
```

1) Create separate columns for sample ID and bottle, 2) nest data, 3) convert each sample's df to have columns for each **biological replicate**
...
SD.by.BioPCR is the function from earlier (somewhat of a misnomer here, because we're only looking at sd of bottle replicates), which we will use to calculate row-wise standard deviations in a nested dataframe.

```{r getting bottle sds}
SD.bio.output<- SD.by.BioPCR(nested.props$data, length(nested.props$sample))
SD.bio.output <- as.numeric(SD.bio.output[-1])

# check output 
length(which(is.na(SD.bio.output) ==T)) # 783 observations
```
