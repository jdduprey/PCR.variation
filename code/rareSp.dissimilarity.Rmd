...
# means of BCDs for each PCR rep combo, separated by threshold proportion
unnested.means<- unnested %>% group_by(unnested$propThresh) %>% summarise_at(c("1.2", "2.3","1.3"), mean)
colnames(unnested.means) <- c("propThresh", "mean1.2", "mean2.3", "mean1.3")
``` 

Now plot. X axis = threshold proportion, y axis = mean BCD across samples (separated by PCR replicate combination)

```{r plot}
matplot(unnested.means$propThresh, unnested.means[,2:4],
        type = "l", xlab = "proportion cutoff",
        ylab="mean BCD",
        ylim=c(0,1), col = c("red", "green","blue"))

legend("topright", legend=c("1.2mean", "2.3mean", "1.3mean"), lty = c(1,2,3),
       col=c("red", "green","blue"), cex=0.8)
```

So what we really want to know is what proportion threshold minimizes the BCD... all 3 lines seem to flatten around a BCD of 0.51, or proportion cutoff of 0.08
