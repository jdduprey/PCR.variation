...
output <-   HashProp_Bray(full.proportion.reads, unique(full.proportion.reads$bio),seq(0.00001, 0.1, 0.0001), c(4:6))
```
...
```{r graph}
matplot(unnested$propThresh, unnested[,1:3], 
        xlab="Hash Proportion Threshold",
        ylab= "BCD") # prop threshold vs. pairwise dist
```

So what we really want to know is what proportion threshold minimizes
