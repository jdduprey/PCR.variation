---
title: "Instead of eDNA index values, calculate Hash proportions (& eDNA index) within samples"
...
# Overview: This code creates 3 csvs: 
  ...
  3) PCR.index.clean.csv (line 189): This code calculates the eDNA index for each Hash within a PCR triplicate.
  4) PCR.duplic.proportions.csv (line 260). This code calculates the proportions for each Hash within a PCR duplicate. These duplicate include true duplicates AND samples that had one PCR replicate removed in the earlier low read quality control step. 
...
by.tech.table <- by.tech.table %>% 
  mutate(bio = gsub("_","", by.tech.table$bio)) %>% 
  mutate(sample = gsub("_","", by.tech.table$sample))
...
# source('.../code/eDNA indexing/helen.eDNA_index.R') # modified index function 
