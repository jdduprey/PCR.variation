...
# Overview: This code creates 3 csvs: 
  1) PCR.proportions.csv (line 66). This code calculates the proportions for each Hash within a PCR triplicate without removing PCR replicates with low reads
  2) PCR.proportions.clean.csv (line 121). This code calculates the proportions for each Hash within a PCR triplicate AFTER removing PCR replicates with low reads
  3) PCR.duplic.proportions.csv (line 199). This code calculates the proportions for each Hash within a PCR duplicate. These duplicate include true duplicates AND samples that had one PCR replicate removed in the earlier low read quality control step. 
...
all.nReads.sums <- triplicate.data.allreads %>%  group_by(sample) %>%
...
So taking a break here - we now have biological replicates that need one PCR replicate removed because it's read was two low. So in essence, these samples are now duplicates. We can combine these samples with our true duplicates to make a triplicate and a duplicate table. 
...
We'll also make the triplicate proportion table here. 
...
# something like this to get hash proportions between bio replicates - but i think
# we want to sum them accross technical replicates first? 
...
Now we're gonna check, just to make sure, that no reads from the true duplicates are really low. This code chunk is the same as code chunks in lines 66 and 84 above.
...
PCR.duplic.proportions <- standard.props %>%
  ...
  select(Miseq_run, bio, tech, Hash, PropRead) %>% 
  pivot_wider(names_from=tech, values_from=PropRead, values_fill = 0) %>% 
  rename(c("PCR1_prop" = "1", "PCR2_prop" = "2"))
