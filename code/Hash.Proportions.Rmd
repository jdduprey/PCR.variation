...
First get raw ASV data. Note that the input data should be formatted as shown:
  Miseq_run        sample                                     Hash nReads
1         1 PO20170311F.1 11a112ced1c85e620f6d97d82281a1bb58614e67  17099
2         1 LL20170311E.2 11a112ced1c85e620f6d97d82281a1bb58614e67  30337
...
# we should combine these into the code above to avoid doing it twice
by.tech.table <- by.tech.table %>% 
  mutate(bio = gsub("_", "", by.tech.table$bio)) %>% 
  mutate(sample = gsub("_", "", by.tech.table$sample))

# does the following code even do anything (since we're not saving it to an object)??? also, I'm not sure the code is right bc the result is wacky
# check for repeats 
by.tech.table %>%
...
PCR.proportions <- by.tech.table %>%
  filter(bio %in% three_reps$bio) %>% # only take bio reps with three PCR reps
...
triplicate.data.allreads <- by.tech.table %>% # why do we do this every time instead of using three_reps?
