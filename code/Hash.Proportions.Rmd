...
PCR.proportions <- by.tech.table %>%
  ...
  group_by(bio, tech, Miseq_run) %>% # removed Miseq_run from group_by() i think this will solve our duplicate hash problem -joe
  mutate(PropRead = nReads/sum(nReads)) %>%
  select(bio, tech, Hash, PropRead, Miseq_run) %>%
...
# Testing a different structure here - Kai
long_props <- by_tech %>%
  group_by(bio, tech) %>%
  mutate(prop = reads / sum(reads)) %>%
  select(bio, tech, seq_run, hash, reads, prop)

write_csv(long_props, "../data/long_PCR_props.csv")
```
...
PCR.proportions.clean <- true.triplicate.data %>%
  group_by(bio, tech, Miseq_run) %>% 
  mutate(PropRead = nReads/sum(nReads)) %>%
  select(bio, tech, Hash, PropRead, Miseq_run) %>%
  pivot_wider(names_from=tech, values_from=PropRead, values_fill=0) %>% 
  dplyr::rename(c('PCR1_prop' = '1', 'PCR2_prop' = '2', 'PCR3_prop' = '3'))%>%    group_by(bio, Hash, Miseq_run)%>% 
=======
...
write_csv(PCR_props.clean, "../data/PCR.proportions.clean.csv")
...
<<<<<<< HEAD
  group_by(bio, tech, Miseq_run) %>% # removed Miseq_run 8/27, see above 
  mutate(PropRead = nReads/sum(nReads)) %>%
  select(bio, tech, Hash, PropRead, Miseq_run) %>% 
  pivot_wider(names_from=tech, values_from=PropRead, values_fill = 0) %>% 
  dplyr::rename(c("PCR1_prop" = "1", "PCR2_prop" = "2")) %>% group_by(bio, Hash, Miseq_run)%>%  summarise_at(c("PCR1_prop", "PCR2_prop"), mean)
