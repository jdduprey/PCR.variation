...
SDcutoffs <- function(df, cutoffs){
  ...
  SD.dists <- matrix(NA, ncol=length(cutoffs), nrow=length(df$Hash),length(cutoffs)) 
    ...
    for(i in 1:length(cutoffs)){
      ...
      lognormal.means[i,3] <- 1 - (length(sub.tib$Hash)/length(meanIndex.acrossPCR$Hash)) # column 3 is % of UNIQUE hashes excluded
...
output<- SDcutoffs(trip.index,seq(0, 0.25, 0.01))
...
SDvsProp.plot <-  ggplot() +  theme_minimal() + geom_line(mapping = aes(x = output[[1]][,1], y = output[[1]][,2]), size = .5, color = "red3") + theme(legend.position = "none") + theme(axis.title.y = element_text(color = "red3")) + scale_y_continuous(name = "Mean SD Across PCRs") + xlab("Cutoff") 

# 2) Plot percent of unique hashes chopped vs. cutoff
ChoppedvsProp.plot <-  ggplot() +  theme_minimal() + geom_line(mapping = aes(x = output[[1]][,1], y = output[[1]][,3]*100), size = .5, color = "darkmagenta") + theme(legend.position = "none") + theme(axis.title.y = element_text(color = "darkmagenta")) + scale_y_continuous(name = "% Unique Hashes Removed") + xlab("Cutoff")
