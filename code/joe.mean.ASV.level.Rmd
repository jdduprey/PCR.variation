...
# do all the proportion add up to 1? they do! 
...
Make and run function that gets SDs across bio reps for each ASV within a sample. I'm rewrite this because
I can't get Helen's function to work :( 
```{r}
...
######## need to make two SD on the same plot i guess 
SD.tech.output$StanDev <- 'techsd'
bio.prop$StanDev <- 'biosd'

histdf <- rbind(SD.tech.output$techsd, bio.prop$biosd)

# how do we handle which site.dates to keep, it appears some have only one biological replicate remaining 
...
Questions: 
1) is it okay to average SDs across ASVs within a sample? 
2) When taking the mean of the standard deviations across ASVs, should I be taking a normal mean or a lognormal mean? this applies both within the function, and taking the final mean of variation for biological and PCR replicates combined. 
3) Also keep in mind that this is not discriminating between samples with different numbers of biological replicates.

```{r isolate bio var}
mean(SD.bio.tech.output) 
mean(SD.tech.output$StanDev)
```

So, that's all wrong...tech variability shouldn't be higher than tech+bio variability. Need to revisit how we can compare these two sources of variability. 


```{r}
length(SD.bio.tech.output)
length(SD.tech.output$StanDev)
```
