...
get.PCRn <- long.props %>% 
  ...
  #group_size() 
  summarise(meanprop=sum(Proportion)/3) #take the mean can this be done with mean()
  
    
nesttest <- nest(get.PCRn, data=!biorep)

# do all the proportion add up to 1? they don't whats going on! 
...
ggplot(hist.df, aes(x=StanDev)) + 
    geom_histogram(data=subset(hist.df, Division == 'tech'),fill = "red", alpha = 0.4, bins=50) +
    geom_histogram(data=subset(hist.df, Division == 'bottle'),fill = "blue", alpha = 0.4, bins=50) +
