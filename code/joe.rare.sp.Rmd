...
filter_proprank <- function(df, proportion, use_rank=TRUE) {
  ...
  rows_to_zero <- original.obs %>%
    filter(PropRead > 0) %>%
    arrange(PropRead) %>%
    filter(row_number() <= proportion) # * n()  # should this be great or less than *******
...
test <- filter_proprank(full.proportion.long, 50000, use_rank=TRUE)
```
...
for (i in x){
  ...
  dat <- filter_proprank(full.proportion.long, i, use_rank=TRUE) 
