...
bray.mat.test <- bray_by_sample(full.proportion.reads, unique(full.proportion.reads$bio), c(3:5))
...
filter_proprank <- function(df, proportion, use_rank=TRUE) {
  ...
  original.rows <- nrow(greater.than.zero)
  print(original.rows)
  
  if (use_rank==FALSE) {
    result.long <- greater.than.zero %>%
    filter(greater.than.zero$PropRead >= proportion) 
    
    remaining.rows <- nrow(result.long)
    
    prop.eliminated <- (original.rows - remaining.rows)/original.rows
    print(remaining.rows)
    print('--------------')
    print(prop.eliminated)
    print('^^^^')
  } 
  ...
  BC.by.bio <- as.data.frame(bray_by_sample(result.wide, unique(result.wide$bio), c(3:5)))
  ####
  BC.by.bio$cutoff <- proportion # show which prop cutoff it came from on the big df 
  ...
  BC.by.bio$prop.elim <- prop.eliminated
    
  return(BC.by.bio)
  
}

test <- filter_proprank(full.proportion.long, 0.02, use_rank=FALSE)
```

```{r}

x <- seq(0,0.03, by=0.0001)
...
```
```{r}
ggplot(eily.plot.df, aes(x=prop.elim, y=mean.all)) + geom_point()
```

```{r}
ggplot(eily.plot.df, aes(x=cutoff, y=prop.elim)) + geom_point()

```
```{r}
ggplot(eily.plot.df, aes(x=cutoff, y=mean.all)) + geom_point()

```

```{r}
ggplot(eily.plot.df, aes(x=cutoff, y=mean.bio)) + geom_point()
