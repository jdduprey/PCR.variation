...
# d) nRun a string object or vector of strings of Miseq run names
PCR.bcd <- function(df, bottles, PCR.cols, nRun){
  bcds <-  c("start") # will hold all bray curt distances
  
  print(nrow(df))
  
  df <- df %>%
    filter(Miseq_run %in% nRun)
  ...
  # plot
   his <- hist((as.numeric(bcds[-1])), col = viridis::plasma(3,0.4,0.7), main = "PCR Variation", xlab = "Bray-Curtis Pairwise Distances")
    ...
    print('-----')
    print(mean(as.numeric(bcds[-1]), na.rm=TRUE))
    return(as.numeric(bcds[-1])) 
    return(his)
}

PCR.output <- PCR.bcd(trip.proportion.reads,  unique(trip.proportion.reads$bio), 4:6, c("1","2","3","4","7","8"))
```
...
# nestList = tibble with columns:
#         1. nestList$Miseq_run - shows sequencing run
#         2. nestList$sample - shows sampling event ID (ex, CP201706)
#         3. nestList$data - list of data frames corresponding to each sampling event. Within each df, columns correspond to bioPCR combos (A1, B3, C2 etc) and rows correspond to each Hashes
# nRun = which MiSeq runs you want to analyze

BOTTLE.bcd <- function(nestList, nRun){
 
  nestList <- nestList %>%
    filter(Miseq_run %in% nRun)
  
  sampleVector  <- nestList$sample
  sampleList <- nestList$data
  ...
  ### Getting rid of samples with only one bottle
      ...
      if(( length(which(is.na(onebottle) == F)) != 0) == T){ # basically, if there were any single bottle samples 
         onebottle <- onebottle[!is.na(onebottle)] # remove NAs
        sampleList <- sampleList[-onebottle] # remove the one bottle elements from list
       }
...
bottle.output <- BOTTLE.bcd(nested.props, c("1","2","3","4","7","8"))
```
