...
find_bottle_BCDs <- function(df) {
  ...
  for (j in 1:length(df$site)) {
    hashes <- df$data[[j]]  # A data frame corresponding to one sampling event
    sub_tib <- hashes %>% select(!c(hash))
    flip_tib <- t(sub_tib)
    ...
    bcd.vector <- as.numeric(na.omit(as.vector(dis_mat))) # remove intra-bottle comparisons
...
bottle.output <- find_bottle_BCDs(kai_PCR_props)
```
