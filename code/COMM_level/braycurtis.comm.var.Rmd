...
```{r Create and run function to calculate BCDs by bottle; Plot}
# Computes pairwise Bray-Curtis dissimilarities among technical replicates from 
# the same biological sample. Returns a vector of those values and a histogram.
#
# Args:
...
PCR.bcd <- function(df, seq_runs = unique(df$seq_run)) {
  df <- df %>% 
  ...
  i <- 1  # For keeping track of the bcds index where we can next add values
  
  for (b in bottles) {
    hashes <- t(df[df$bio == b, PCR_cols]) # to get all PCR props from that one sample
    dis <- vegdist(hashes)
    bcds[i : (i + 2)] <- as.vector(dis) # takes all values of the vegdist output
    print(i)
    i <- i + 3  # GENERALIZE
  }
  ...
  his <- hist(bcds,
              ...
              main = "PCR Variation",
              xlab = "Bray-Curtis Pairwise Dissimilarities")

  return(bcds)
  return(his)
}

PCR.output <- PCR.bcd(kai_PCR_props, 1)  # TODO : FIGURE OUT WHAT'S UP WITH 5 AND 6 AND ALSO DIFFERNECES BTW SEQ RUNS
# mean = 0.334
```
