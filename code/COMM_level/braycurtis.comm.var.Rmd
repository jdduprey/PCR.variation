...
long.props <- trip.proportion.reads %>% 
  mutate(sample = gsub('.{1}$', '', bio)) %>% # remove the A (or B, C, etc.) in CP201706A
  mutate(bottle = stri_sub(bio,-1)) %>% # create column with bottle letter
  pivot_longer(cols = c(PCR1_prop,PCR2_prop,PCR3_prop), names_to = "PCR", values_to = "Proportion")

get.PCRn <- long.props %>% 
  mutate("PCRn" = stri_sub(PCR,4,-6)) %>% 
  select(-c(bio,PCR)) # columns become: Hash, sample, bottle, Proportion, PCRn

nested.props <- get.PCRn %>% 
  unite(bottle, PCRn, col = "bio.PCR", sep = ".") %>% # new column has values A.1, A.2, etc.
  nest(data = c(bio.PCR,Hash,Proportion)) # columns are now: sample, data (with each value being a tibble)

for(i in 1:length(nested.props$sample)) {
  
  nested.props$data[[i]] <- nested.props$data[[i]] %>% 
    group_by(bio.PCR, Hash) %>% # WHAT ARE THIS AND THE NEXT LINE DOING?
...
# list = list of data frames corresponding to each sampling event. Within each df, columns correspond to bioPCR combos (A1, B3, C2 etc) and rows correspond to individual Hashes
# SampleVector is vector of sampling events (PO20170311, etc)
BOTTLE.bcd <- function(list, SampleVector) {
  bcds <- "start"
 
  # Get rid of samples with only one bottle
  ...
  for(i in 1:length(SampleVector)) {
    a <- list[[i]]
    if(((ncol(a) - 1) / 3) == 1) { # replicates with only 1 bottle, USES ASSUMPTION THAT IT'S ALWAYS TRIPLICATES
