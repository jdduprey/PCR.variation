...
classes <- c("Mammalia","Actinopteri", "Bivalvia", "Polychaeta", "Phaeophyceae","Dictyochophyceae", "Dinophyceae")

class_subset_indices <- which(reads_annotated_no_drops$class %in% classes)

reads_annotated_class_subset <- reads_annotated_no_drops[class_subset_indices,]

ggplot(reads_annotated_class_subset, aes(x = class, y = CV)) +
  geom_boxplot() +
  scale_x_discrete(guide = guide_axis(angle = 90))
```
...
prop.drops <- function(wide_df, classes){

proportion_drops <- data.frame(class = classes, proportionDrops = NA)
for(i in 1:length(classes)){
  df <- wide_df[wide_df$class == classes[i],]
...
output_df <- prop.drops(wide_reads_annotated, classes)

ggplot(output_df, aes(x = class, y = proportionDrops)) +
  geom_bar(stat="identity") + 
   scale_x_discrete(limits = classes, guide = guide_axis(angle = 90)) 
```
