...
Where amplification efficiency of these species is high, the pattern can be striking: high read numbers in one replicate, zero in another [Figure 2]. But we expect the greatest number of dropouts to be associated with rare, low amplification efficiency species - these molecules, if included in a technical replicate, may fail to amplify at all.  **this last sentence may not be true - circle back HC**
...
[The Shelton model], which links a species' metabarcoding output (set of observed sequenced reads) to its original template DNA concentration by modeling laboratory processes, functions when variation among technical replicates is reasonably low. Unexpectedly high variability causes the model to fail; we propose this variability is a function of original DNA concentration, amplification efficiency, and sample read depth for any given species. Mock communities let us test this -- they have known concentrations and well-estimated amplication efficiencies. 

As part of the California Cooperative Oceanic Fisheries Investigations (CalCOFI), Gold et al. (IN PREP) used morphological and molecular methods to analyze the response of ichthyoplankton in the California Current Large Marine Ecosystem to ocean warming. Ichthyoplankton samples were collected during CalCOFI research cruises over two decades (1996; 1998-2019). Part of each sample was preserved in Tris-buffered 95% ethanol for metabarcoding using the Mifish primer set. The remaining part was preserved in sodium borate-buffered 2% formaldehyde for microscopy-derived species identification and abundance (number of larvae per species per jar). **I might be (probably am) missing important details here HC** Amplification efficiency was estimated for each species during PCR cycling in the metabarcoding process.
...
**possibly instead, we get Bray-Curtis among technical replicates (comm-level) for datasets using different primers...see if specific primers (rare) == more distant tech rep communities...we don't need original biomass or amp eff for this**
