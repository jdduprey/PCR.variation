...
make_seasonal_df_phyl <- function(wide_df, taxa_list, taxa_level){
  ...
  seasonal_drops <- data.frame(taxa_groups = taxa_list, "mean_read_depth_sp" = NA,
                               "mean_read_depth_fall" = NA, "Spring"=NA, "Fall"=NA)
  ...
  for(i in 1:length(taxa_list)){
    ...
    for(k in colnames(seasonal_drops)[-c(1:3)]){
      ...
      seasonal_drops[i, k] <- calc_prop
        if(k == "Fall"){ seasonal_drops[i, "mean_read_depth_fall"] <- mean(df_sm$mean_reads) }
      if(k == "Spring"){ seasonal_drops[i, "mean_read_depth_sp"] <- mean(df_sm$mean_reads) }
      }
...
dropouts_by_season_df <- dropouts_by_season_df  %>%
  pivot_longer(
    ...
    na.omit() %>% mutate(Season = case_when(Season == 1 ~ "Spring",
                                            Season == 2 ~ "Fall"))


dropouts_by_season_df$Season <- factor(dropouts_by_season_df$Season, levels = c("Spring", "Fall"))
