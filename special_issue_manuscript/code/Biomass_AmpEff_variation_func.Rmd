...
total_binned_df <- data.frame(input_df,abundance_bin=cut(input_df$abundance,as.numeric(quantile(input_df$abundance)),include.lowest=TRUE), 
ampef_bin = cut(input_df$amp_ef,unique(as.numeric(quantile(input_df$amp_ef))),include.lowest=TRUE)) # sort abundance and amp efficiency into quantiles with all input data
...
print(levelplot(dropout_proportion ~ `2`*`1`, 
          scales=list(x= list(at=sort(unique(output$`2`))),
                      y= list(at=sort(unique(output$`1`)))),
...
return(cbind(propdrops, unique(cbind(bin_levels_list$abundance_bin, bin_levels_list$ampef_bin))))
}
...
Joe_data <- read.csv(here("special_issue_manuscript","Data", "gold_example_data.csv"))

output <- TechDrops.vs.CountsAmps(input_df = Joe_data)
```
