...
total_binned_df <- data.frame(input_df,abundance_bin=cut(input_df$abundance,unique(as.numeric(quantile(input_df$abundance, na.rm = T, probs = seq(0, 1, 1/8)))),include.lowest=TRUE),  ampef_bin = cut(input_df$amp_ef,unique(as.numeric(quantile(input_df$amp_ef,na.rm = T, probs = seq(0, 1, 1/8)))),include.lowest=TRUE))  # sort abundance and amp efficiency into quantiles with all input data
