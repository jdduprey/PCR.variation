...
TechDrops.vs.CountsAmps <- function(input_df){
  ...
  total_binned_df <- data.frame(input_df, abundance_bin=cut(input_df$abundance, na.rm =T, breaks = 3),  ampef_bin = cut(input_df$amp_ef, na.rm= T, breaks = 7))  # sort abundance and amp efficiency into quantiles with all input data
...
output <- TechDrops.vs.CountsAmps(input_df = test)
```
