...
# try with some additional kingdoms 
print(unique(reads_annotated_no_drops$kingdom))

reads_annotated_kingdom_amob <- reads_annotated_no_drops %>%
  filter(kingdom %in% c("Viridiplantae", "Stramenopiles", "Haptophyceae", 
                        "Metazoa", "Rhodophyta", "Fungi","Dinoflagellates"))

# kingdoms not included have tiny sample sizes

ggplot(reads_annotated_kingdom_amob, aes(x = kingdom, y = CV)) +
  geom_boxplot()
...
all_king_drops <- prop.drops(wide_reads_annotated, c("Viridiplantae", "Stramenopiles", "Haptophyceae", 
                            "Metazoa", "Rhodophyta", "Fungi","Dinoflagellates") )

ggplot(output_df, aes(x = kingdom, y = proportionDrops)) +
  geom_bar(stat="identity") + 
  scale_x_discrete(limits = c("Metazoa", "Rhodophyta", "Fungi","Dinoflagellates"), guide = guide_axis(angle = 90)) 
...
phyla_bar_df_1 <- by_phylum_drops(wide_reads_annotated, c("Annelida", "Chordata", "Porifera", "Arthropoda",
                                                        "Rotifera", "Mollusca", "Bryozoa", "Cnidaria", 
                                                        "Echinodermata"))

ggplot(phyla_bar_df, aes(x = phylum, y = proportionDrops)) +
  geom_bar(stat="identity") 
```
